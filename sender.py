...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    # TODO IMPLEMENT DROPPED ACK CONTROL
    while win_left_edge < INIT_SEQNO + content_len:
        ...
        cumulativeAck = INIT_SEQNO  # First ack should be INIT_SEQNO + 2 == 7
        firsttx = win_left_edge
        finalAck = INIT_SEQNO + content_len
        # TODO IMPLEMENT DROPPED ACK CONTROL
        while inputs:
            # print("waiting")
            # print(cumulativeAck)
            ...
            if cumulativeAck < firsttx and not read:
                win_left_edge = cumulativeAck
                transmit_one()
                print("retransmitting")
                retransmitting = True

            # Any time the cumulativeAck == Final ack, the message has been sent; safety break
            if cumulativeAck >= finalAck and firsttx >= finalAck:
                win_left_edge = INIT_SEQNO + content_len
                break
            ...
            for s in write:  # Will write to each server that is wanting to initially be written to from inputs
                if win_left_edge < INIT_SEQNO + content_len:
                    firsttx = transmit_entire_window_from(firsttx)
                    win_right_edge += win_size
                    if win_right_edge > finalAck:
                        win_right_edge = finalAck
                    # time.sleep(.5)
                # outputs.remove(s)

            for r in read:  # Will read data from the servers it contacted and that are readable
