...
def receiver(ss, ooo_enabled):
    def get_msg_ack():
        ...
        #############################################
        # Username Check Code
        finalMessage = "" # Keep blank to only print when filled as it will only be filled on the "second" packet
        if msg.len > 1 and msg.len <= 8: # After the first ack for len of msg, and checking the len of the second
            usernameCheck = msg.msg[:3]
            if usernameCheck == "UN@": # If the msg starts with UN@
                nameCheck = msg.msg[3:]
                if nameCheck: # If there is actually text after the UN@
                    username = usernameCheck+nameCheck
                    foundUser = False
                    try: # Making sure the file exists
                        with open("UN.txt", "r") as file:
                            for line in file:
                                if line.strip() == username.strip():
                                    finalMessage = "Username found: " + username
                                    foundUser = True
                                    break
                            file.close()
                    except FileNotFoundError: # Will creat the file if it doesn't exist
                        createFile = open("UN.txt", "x")
                        createFile.close()
                    if foundUser == False: # If the username was not found in the file, open and write to it
                        f = open("UN.txt", "a")
                        f.write(username + "\n")
                        f.close()
                        finalMessage = "Username not found"
                else: # If the content after UN@ was blank
                    finalMessage = "Username not found"
            else: # If the message didn't start with UN@
                finalMessage = "Username not found"
        elif msg.seq != 5: # Hard coded to match the initial sequence from the sender.py
            finalMessage = "Username not found"
